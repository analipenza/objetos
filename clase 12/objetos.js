...
//Vamos a crear un arreglo de películas, 
//cada película tiene que ser un objeto con las propiedades: titulo, 
//un rating, y loHasVisto. Usa un for loop para iterar sobre el arreglo e 
//imprimir un resultado en la consola que se vea como esto:
...
var arreglopeliculas= [peliculas]

var peliculas = {    //OMA tengo que hacer varios objetos o la idea es que de un objeto se vaya llenando por el user???
    titulo: 'el gran pez',
    rating: '5 estrellas',
    lohasvisto: 'lo he visto',
}


for (var i = 0; i < arreglopeliculas.length; i++) {  
    if (arreglopeliculas[i].length >= 0) {  
      
        for (var propiedades in peliculas){
            if (peliculas[propiedades][i] == 'lo he visto'){
                console.log('Has visto ' + arreglopeliculas[i] + peliculas.rating(i) + peliculas.rating[i])
            }else {
                console.log('no lo has visto ' + arreglopeliculas [i] + peliculas.rating(i) + peliculas.rating[i])
        }
